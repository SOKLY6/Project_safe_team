...
from app.models.qr_code import QR_code
...
async def qr_generation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with async_session() as session:
        ...
        qr_result = await session.execute(
            select(QRCode)
            .filter(QRCode.user_id == user.id, QRCode.used == False, QRCode.expires_at > datetime.utcnow())
            .order_by(QRCode.created_at.desc())
        )
        qr_code_record = qr_result.scalars().first()

        if qr_code_record is None:
            await update.message.reply_text('У вас нет доступных QR-кодов. Сгенерируйте новый.')
            return

        qr_code_str = qr_code_record.code  # Именно по этому "хэшу" строим QR-картинку!
        ...
        lifetime = (qr_code_record.expires_at - qr_code_record.created_at).total_seconds() // 60
