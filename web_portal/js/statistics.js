...
let currentFilters = { dateFrom: null, dateTo: null, status: 'all', organization: 'all', searchName: '' };
...
async function loadMoreEvents() {
  ...
  if (startIdx >= filteredEvents.length) {
    hasMoreData = false;
    showNoMoreData();
    return;
  }
  ...
  hasMoreData = endIdx < filteredEvents.length;
  isLoading = false;
  hideLoadingMore();
...
function applyFilters() {
  ...
  filteredEvents = allEvents.filter(event => {
    if (currentFilters.dateFrom) {
      ...
      if (eventDate < currentFilters.dateFrom) return false;
    }

    if (currentFilters.dateTo) {
      const eventDate = new Date(event.timestamp).toISOString().split('T')[0];
      if (eventDate > currentFilters.dateTo) return false;
    }

    if (currentFilters.status !== 'all') {
      const isGranted = currentFilters.status === 'granted';
      if (event.access_granted !== isGranted) return false;
    }
...
function clearFilters() {
  ...
  currentFilters = { dateFrom: null, dateTo: null, status: 'all', organization: 'all', searchName: '' };
  applyFilters();
}
...
function renderEventsPage(events) {
  events.forEach(event => {
    eventsTbody.appendChild(createEventRow(event));
  });
}
...
function createEventRow(event) {
  ...
  row.innerHTML = `
    ...
    <td><strong>${escapeHtml(event.user_name)}</strong></td>
    ...
    <td><small class="text-muted">${escapeHtml(event.scanner_name)}</small></td>
  `;
...
function exportToCSV() {
  ...
  link.href = url;
  link.download = `statistics_${dateStr}.csv`;
